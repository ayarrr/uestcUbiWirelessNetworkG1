...
import argparse
import logging
from dataset_lu import BRCDataset
...
def parse_args():
    ...
    model_settings.add_argument('--max_pos_len', type=int, default=500,
                                help='max length of pos')
    ...
    path_settings.add_argument('--test_files', nargs='+',
                               ...
                               # default=['E:/test2_preprocessed/test2_preprocessed/test2set/search.test2.json'],
    ...
    path_settings.add_argument('--pretrained_word_path',
                               default='../data/sgns.target.word-character.char1-2.dynwin5.thr10.neg5.dim300.iter5',
...
def prepare(args):
    ...
    brc_data = BRCDataset(args.max_p_num, args.max_p_len, args.max_q_len,args.max_pos_len,
                          args.train_files, args.dev_files, args.test_files)
    vocab = Vocab(lower=True)
    for word in brc_data.word_iter('train'):
        vocab.add(word)

    for pos in brc_data.pos_iter('train'):
        # print(pos)
        vocab.add_pos(pos)
    ...
    logger.info('After filter {} tokens, the final vocab size is {}'.format(filtered_num,
                                                                            vocab.size()))
    print(vocab.pos_size())
    unfiltered_vocab_pos_size = vocab.pos_size()
    vocab.filter_pos_by_cnt(min_cnt=2)
    filtered_pos_num = unfiltered_vocab_pos_size - vocab.pos_size()
    logger.info('After filter {} pos, the final pos vocab size is {}'.format(filtered_pos_num,
                                                                                vocab.pos_size()))
    ...
    # vocab.randomly_init_embeddings(args.embed_size)
    vocab.randomly_init_pos_embeddings(args.pos_embed_size)
...
def evaluate(args):
    ...
    brc_data = BRCDataset(args.max_p_num, args.max_p_len, args.max_q_len, args.max_pos_len,dev_files=args.dev_files)
    logger.info('Converting text into ids...')
    brc_data.convert_to_ids(vocab)
    ...
    dev_batches = brc_data.gen_mini_batches('dev', args.batch_size,
                                            pad_id=vocab.get_id(vocab.pad_token), pad_pos_id = vocab.get_pos_id(vocab.pad_token),shuffle=False)
