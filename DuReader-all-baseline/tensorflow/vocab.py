...
class Vocab(object):
    ...
    def get_pos_id(self, token):
        token = token.lower() if self.lower else token
        try:
            return self.pos2id[token]
        except KeyError:
            return self.pos2id[self.unk_token]
    ...
    def add_pos(self, token, cnt=1):
        token = token.lower() if self.lower else token
        # print(token)
        if token in self.pos2id:
            idx = self.pos2id[token]
        else:
            idx = len(self.id2pos)
            self.id2pos[idx] = token
            self.pos2id[token] = idx
        if cnt > 0:
            if token in self.pos_cnt:
                self.pos_cnt[token] += cnt
            else:
                self.pos_cnt[token] = cnt
        # print(self.pos2id)
        return idx
    ...
    def filter_pos_by_cnt(self, min_cnt):
        filtered_tokens = [token for token in self.pos2id if self.pos_cnt[token] >= min_cnt]
        # rebuild the token x id map
        self.pos2id = {}
        self.id2pos = {}
        for token in self.initial_tokens:
            self.add_pos(token, cnt=0)
        for token in filtered_tokens:
            self.add_pos(token, cnt=0)
    ...
    def randomly_init_pos_embeddings(self, embed_dim):
        self.pos_embed_dim = embed_dim
        self.pos_embeddings = np.random.rand(self.pos_size(), embed_dim)
        for token in [self.pad_token, self.unk_token]:
            self.pos_embeddings[self.get_pos_id(token)] = np.zeros([self.pos_embed_dim])
    ...
    def convert_pos_to_ids(self, tokens):
        ...
        #     pos_vec = []
        #     for char in token:
        #         pos_vec.append(self.get_pos_id(char))
        #     vec.append(pos_vec)
        # return vec
        vec = [self.get_pos_id(label) for label in tokens]
        return vec
