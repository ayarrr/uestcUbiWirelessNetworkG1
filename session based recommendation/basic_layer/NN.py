...
class NN(object):
    ...
    def optimize_normal(self, loss, params):
        ...
        self.optimizer = tf.train.GradientDescentOptimizer(self.lr)# 采用随机梯度下降
        grads_and_vars = self.optimizer.compute_gradients(loss, params)#计算各个变量的梯度，loss为损失，param为各个参数列表返回一个以元组(gradient, variable)组成的列表
        ...
        inc = self.global_step.assign_add(1)#计步
        optimize = None
